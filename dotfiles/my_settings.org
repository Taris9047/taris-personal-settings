...
- [[#command-line-aliases][Command line aliases]]
  ...
  - [[#a-few-more-fun-aliases][A few more fun aliases]]
- [[#basic-functions-for-everyday-use][Basic Functions for everyday use]]
  ...
  - [[#editor-aliases---for-gedit][Editor aliases - for gedit]]
...
- [[#shell-extension][Shell extension]]
  - [[#starship][Starship]]
  - [[#fzf---the-fuzzy-finder][FZF - The fuzzy finder]]
...
Latest fix redirects you to the most recent TMUX session if exists.
...
#+begin_src fish :tangle my_settings_fish
alias tmuxkillall='tmux kill-server'
if set -q SSH_CLIENT; or set -q SSH_TTY; or test $TERM == "*screen*"
    if type -q tmux;
        tmux new
    end
    builtin exit
...
Running 'git pull' each time is kind of a chore. Let us run it every time we log in.
--> Actually, deprecated here. Calling ```git pull``` every login sometimes causes a significant drag.
...
if test -f "$LS_MODE_SAVE"
    ...
    set LS_MODE "$DEF_LS_MODE"
    touch "$LS_MODE_SAVE"
    echo "$DEF_LS_MODE" >"$LS_MODE_SAVE"
end
#+end_src
...
set_eza_as_ls() {
    if [ -x "$(command -v eza)" ]; then
        ...
        alias l='eza -hF --git --icons=auto'
        alias lt='eza -hF --git --icons=auto -T'
        alias l.='eza -halF | egrep "^\."'
        echo 'eza' >"$LS_MODE_SAVE"
    else
        printf 'It seems eza has not yet installed!!\nUsing the good old ls\n'
        set_ls_as_ls
    fi
}
...
set_ls() {
    ...
    "*")
        printf "  %b Activating '%s' mode.\n" "${right_arrow_symbol}" "${LS_MODE}"
        printf "    Select one of: lsd, exa, eza, ls\n"
        ;;
    esac
}
...
function set_exa_as_ls
    if type -q exa
        ...
        printf 'exa does not exist! No changes applied\n'
    end
end
...
function set_lsd_as_ls
    if type -q lsd
        ...
        printf 'lsd does not exist! No changes applied\n'
    end
end

function set_eza_as_ls
    if type -q eza
        sleep "$line_delay"
        alias ls='eza -hF --git --icons=auto'
        alias ll='eza -halF --git --icons=auto'
        ...
        printf 'eza does not exist! No changes applied\n'
    end
end
...
set GOOGLE_PHOTOS "$HOME/.google-photos"
set ONE_DRIVE "$HOME/.onedrive"
set ICLOUD_DRIVE "$HOME/.iclouddrive"
#+end_src
...
** Editor aliases - for gedit

In recent Linux, gedit is dropped and replaced with newer(?) and better(hopefully) editors. And their executable names sometimes hard to remember. So, let's mak some aliases for them.
...
if [ ! -x "$(command -v gedit)" ]; then
  ...
  # In case of modern Gnome..
  if [ -x "$(command -v gnome-text-editor)" ]; then
    alias edit='gnome-text-editor'
  fi

  # In case of Cosmic
  if [ -x "$(command -v cosmic-text-editor)" ]; then
    alias edit='cosmic-text-editor'
  fi

  # In case of Linux Mint
  if [ -x "$(command -v xed)" ]; then
    alias edit='xed'
  fi
  
else
  alias edit="$(command -v gedit)"
fi
...
#+NAME: prog_golang
#+begin_src shell :tangle my_settings_bash
SYSTEM_GOROOT="/usr/local/go"
if [ -d "${SYSTEM_GOROOT}" ]; then
...
if [ -x "$(command -v $GOROOT/bin/go)" ]; then
...
A shell integration for FZF fuzzy finder integration. More will come.
...
if type -q figlet
    if type -q lolcat
        figlet -f future -w (/usr/bin/tput cols) "$hostname" | lolcat
    else
        figlet -f future -w (/usr/bin/tput cols) "$hostname"
    end
end
#+end_src
...
*** Fish
#+begin_src fish :tangle my_settings_fish
set VPN_PATH "/opt/cisco/secureclient/bin/vpn"
if type -q "$VPN_PATH"
    alias ucsc_vpn="$HOME/.settings/bin/vpn_connect_ucsc.sh"
end
#+end_src
