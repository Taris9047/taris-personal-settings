...
- [[#mount-clouds][Mount Clouds]]
  - [[#google-drive-and-photos][Google Drive and Photos]]
  - [[#onedrive][Onedrive]]
...
if [ -x "$(command -v rclone)" ]; then
	...
	if grep -qs "${GOOGLE_DRIVE}" '/proc/mounts'; then
		...
		printf '%b RClone for Google Drive not set up yet!\n' "${check_symbol}"
	else
    ...
    if [ ! -d "${GOOGLE_PHOTOS}" ]; then
        printf '%b Google photos mount point not found! Skipping...' "${check_symbol}"
    else
        if grep -qs "${GOOGLE_PHOTOS}" '/proc/mounts'; then
            printf '%b Google Photos already mounted at %s\n' "${check_symbol}" "${GOOGLE_PHOTOS}"
            sleep "${line_delay}"
        elif [ ! -f "${HOME}/.config/rclone/rclone.conf" ]; then
            printf '%b RClone for Google Photos not set up yet!\n' "${check_symbol}"
        else
            if [ ! -z "" ]; then
                printf '%b Mounting Google Photos to %s\n' "${check_symbol}" "${GOOGLE_PHOTOS}"
                rclone mount google-photos: --vfs-cache-mode full "${GOOGLE_PHOTOS}" &
            fi
        fi
    fi
fi
