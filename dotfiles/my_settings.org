...
JUPYTER_TMP_DIR="$HOME/.jupyter_tmp"
if grep -q "snap" <<< "$BROWSER"; then
    printf '%b System browser seems to be snap version. Exporting some envs for Jupyter\n' "${check_symbol}"
    export JUPYTER_DATA_DIR="$JUPYTER_TMP_DIR"
    export JUPYTER_RUNTIME_DIR="$JUPYTER_TMP_DIR"
fi
#+end_src
...
set JUPYTER_TMP_DIR "$HOME/.jupyter_tmp"
if grep -q "snap" <<< "(command -v firefox)"; then
    printf '%b System browser seems to be snap version. Exporting some envs for Jupyter\n' "${check_symbol}"
    set -Ua JUPYTER_DATA_DIR "$JUPYTER_TMP_DIR"
    set -Ua JUPYTER_RUNTIME_DIR "$JUPYTER_TMP_DIR"
end
#+end_src
