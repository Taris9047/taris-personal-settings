...
if grep -q "snap" <<< "$BROWSER"; then
    printf '%b System browser seems to be snap. Exporting some envs for Jupyter\n' "${check_symbol}"
    export JUPYTER_DATA_DIR=/home/$USER/.jupyter_tmp
    export JUPYTER_RUNTIME_DIR=/home/$USER/.jupyter_tmp
fi
#+end_src
...
if grep -q "snap" <<< "(command -v firefox)"; then
    printf '%b System browser seems to be snap. Exporting some envs for Jupyter\n' "${check_symbol}"
    set JUPYTER_DATA_DIR /home/$USER/.jupyter_tmp
    set JUPYTER_RUNTIME_DIR /home/$USER/.jupyter_tmp
end
#+end_src
