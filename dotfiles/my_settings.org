...
- [[#command-line-aliases][Command line aliases]]
  ...
  - [[#git-completions-for-zsh][Git Completions for Zsh]]
  - [[#termbin][Termbin]]
...
if [ -x "$(command -v tmux)" ] && ! stringcontain 'screen' "$TERM" && ! stringcontain 'tmux' "$TERM" && [ -z "$TMUX" ]; then
...
Somehow, zsh in many distro packages does not come with git completions and makes it a bit clunky to use with git. So, let's install zsh git-completion from git's Github repo! Referenced [[https://oliverspryn.medium.com/adding-git-completion-to-zsh-60f3b0e7ffbc][Oliver Spryn's web site]].

TODO We might deprecate this part? Oh-my-zsh does everything this part provides...
...
emacsclient_cmd="$(command -v emacsclient)"
unalias em emacs emacsclient 2> /dev/null
if [ ! -z "$(cat /etc/os-release | grep ID | grep elementary)" ]; then
    alias emacs="env XLIB_SKIP_ARGB_VISUALS=1 ${emacsclient_cmd} -c -a 'emacs' "
    alias em="${emacs_cmd} -nw"
else
    alias emacs="${emacsclient_cmd} -c -a 'emacs' "
    alias em="${emacs_cmd} -nw"
fi
#+end_src

#+begin_src shell :tangle my_settings_bash
# Aaaaand some shortcut for emacs
emc() {
	if [ $# -eq 0 ]; then
		emacsclient &
		>/dev/null &
...
#+NAME: prog_bashtop
#+begin_src shell :tangle my_settings_bash
if [ -x "$(command -v $HOMEBREW/bin/pip3)" ] && [ -x "$(command -v $HOMEBREW/bin/bpytop)" ]; then
...
if [ -x "$(command -v starship)" ]; then
    ...
    case "$(echo $MYSHELL)" in
        ,*"bash")
            printf "  %b Running it as bash" "${right_arrow_symbol}"
            eval "$(starship init bash)"
            ;;
        ,*"zsh")
            printf "  %b Running it as zsh" "${right_arrow_symbol}"
...
if [ -x "$(command -v figlet)" ]; then
    if [ -x $(command -v lolcat) ]; then
        figlet -w "$(tput cols)" "$HOST" | lolcat
    else
        figlet -w "$(tput cols)" "$HOST"
    fi
fi
#+end_src
